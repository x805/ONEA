...
#if defined(NDEBUG)
# error "ONEA cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb04016686e87d3a8c15695c5d36ebbe3394f021460f7d89cb9592700c9bfeea7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ONEA: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ONEA Signed Message:\n";
...
{
    // ONEA: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 68 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 1190000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ONEA: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ONEA: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1503072012, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The times 2017 08 18 Low tech terror in an age of social media";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 68 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04742a0452261a5f00e65a77f01c784ec12495fdb734bf310e91d75c063ee8e0d7368d761803d14e8303a5ba683f8570894a512fd4d472f3d3f15dd0cc2eb3d22e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503072012;
        ...
        assert(block.hashMerkleRoot == uint256("0xe310606ae523a7dfda8a73540135d5f0516f1f82367c2ed3354b8e7d3b1b15bf"));
...
unsigned char pchMessageStart[4] = { 0x55, 0x0b, 0xaf, 0x3e }; // ONEA: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ONEAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ONEAMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ONEAMiner(CWallet *pwallet)
{
    printf("ONEAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("onea-miner");
    ...
    try { loop {
        ...
        printf("Running ONEAMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ONEAMiner, pwallet));
}
